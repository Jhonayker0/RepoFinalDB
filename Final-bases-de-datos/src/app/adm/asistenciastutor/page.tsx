...
export default function AsistenciasUsuarios() {
  ...
  const fetchAsistencias = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/asistencias-usuarios?grupo=${grupo}`);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-100 p-8">
      <div className="bg-white p-6 rounded shadow-md w-full max-w-4xl">
        <h1 className="text-2xl font-bold text-center mb-4">Asistencias de Usuarios</h1>
        <div className="mb-4">
              ...
              <tbody>
                {asistencias.map((asistencia, index) => (
                  <tr key={index} className="text-center">
                    ...
                    <td className="border border-gray-300 px-4 py-2">{asistencia.usuario}</td>
                    <td className="border border-gray-300 px-4 py-2">{asistencia.rol}</td>
